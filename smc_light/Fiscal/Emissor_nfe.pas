...
|----------|-------------|-----------------------------------------------------|
|16/03/2020|WANDER       |Passou a chamar a rotina de tratamento de documentos |
|        27|             |fiscais atualizando a variável global                |
|          |             |g_Numero_do_Movimento com o número do movimento      |
|          |             |para o qual será emitido documento fiscal            |
================================================================================
}
interface
...
type
  ...
  private
    ...
    p_cod_venda: Integer;  // Codigo do movimento para o qual será emitido doc fiscal
  public
    { Public declarations }
...
var
  frm_emissor_nfe2: Tfrm_emissor_nfe2;
...
uses nfe_nota_referenciada,  nfe_tributacao, nfe_financeiro,
  ...
  // Emissao de NFe
  EmissaoDeNFe, u_funcoes, Global_Variables;


procedure Tfrm_emissor_nfe2.btn_atualizar_infoClick(Sender: TObject);
begin
Frm_Inutilizar_Faixa := TFrm_Inutilizar_Faixa.Create(nil);
Frm_Inutilizar_Faixa.ShowModal;
end;

{constructor Tfrm_emissor_nfe2.Create(AOwner: TComponent; iVenda: Integer);
begin
  if iVenda < 1 then
  begin
     //Action := cafree;
...
procedure Tfrm_emissor_nfe2.cxButton11Click(Sender: TObject);
begin
  // Chama a unit responsável por todo o tratamento de documentos fiscais
  //----------------------------------------------------------------------------

  // Tipo de emissão - obrigatório
  if rgtpEmis.ItemIndex = -1 then
  begin
    ShowMessage('Informe o tipo de emissão');
    rgtpEmis.SetFocus;
    exit;
  end;

  // Define o tipo de emissão da NFe
  g_NFE_tpEmis := rgtpEmis.ItemIndex;
...
procedure Tfrm_emissor_nfe2.cxButton12Click(Sender: TObject);
begin
tb_pedido.Show;
end;

procedure Tfrm_emissor_nfe2.cxButton13Click(Sender: TObject);
begin
tb_pedido.Show;
end;

procedure Tfrm_emissor_nfe2.cxButton2Click(Sender: TObject);
begin
frm_dados_transporte := Tfrm_dados_transporte.Create(nil);
frm_dados_transporte.ShowModal;
end;

procedure Tfrm_emissor_nfe2.cxButton3Click(Sender: TObject);
begin
frm_nfe_financeiro := Tfrm_nfe_financeiro.Create(nil);
frm_nfe_financeiro.ShowModal;
end;

procedure Tfrm_emissor_nfe2.cxButton4Click(Sender: TObject);
begin
frm_enviar_email :=Tfrm_enviar_email.Create(nil);
frm_enviar_email.ShowModal;
end;

procedure Tfrm_emissor_nfe2.cxButton5Click(Sender: TObject);
begin
tb_pedido.Show;
end;

procedure Tfrm_emissor_nfe2.cxButton6Click(Sender: TObject);
begin
...
procedure Tfrm_emissor_nfe2.cxButton7Click(Sender: TObject);
begin
frm_nfe_tributacao := Tfrm_nfe_tributacao.Create(nil);
frm_nfe_tributacao.ShowModal;
end;

procedure Tfrm_emissor_nfe2.cxButton8Click(Sender: TObject);
begin
frm_nfe_notas_ref := Tfrm_nfe_notas_ref.Create(nil);
frm_nfe_notas_ref.ShowModal;
end;

procedure Tfrm_emissor_nfe2.FormClose(Sender: TObject;
...
procedure Tfrm_emissor_nfe2.FormShow(Sender: TObject);
begin
     rgtpEmis.itemindex := 0;
end;
