...
type
  ...
  private
    ...
    procedure Tratar_Grupo_B20b_Nfe_Referenciada_ProdutorRural;
...
begin
   ...
   //############### N A O   A G R U P A D O ###################################
    ...
    // tratar isto...
    //if qVENDA.FieldByName('PED_FRETEPORCONTA').AsString = '0' then
    //   Nota.Transp.modFrete := mfContaEmitente
    //else
    //   Nota.Transp.modFrete := mfContaDestinatario;
    //
    //
    // TRANSPORTADOR
    //
    Nota.Transp.Transporta.xNome   := '';
    Nota.Transp.Transporta.CNPJCPF := '';
    Nota.Transp.Transporta.IE      := '';
    Nota.Transp.Transporta.xEnder  := '';
    Nota.Transp.Transporta.xMun    := '';
    Nota.Transp.Transporta.UF      := '';
    //
    {
    qVEICULO_VEI.Close;
    ...
    qVEICULO_VEI.ParamByName('VEI_PLACA').AsString  := qVENDA.FieldbyName('PED_PLACA').AsString;
    qVEICULO_VEI.Open;
    //
    if not qVEICULO_VEI.eof then
    begin
        Nota.Transp.veicTransp.placa := SemEspacos(qVEICULO_VEI.FieldbyName('VEI_PLACA').AsString);
        Nota.Transp.veicTransp.UF    := qVEICULO_VEI.FieldbyName('VEI_UF'   ).AsString;
        Nota.Transp.veicTransp.RNTC  := qVEICULO_VEI.FieldbyName('VEI_ANTT' ).AsString;
        //
        ...
        qTRANSPORTADOR.ParamByName('FOR_CODIGO').AsString  := qVEICULO_VEI.FieldbyName('VEI_PROPRIETARIO' ).AsString;
        qTRANSPORTADOR.Open;
        if not qTRANSPORTADOR.eof then
        begin
           Nota.Transp.Transporta.xNome   := qTRANSPORTADOR.FieldbyName('FOR_RAZAO_SOCIAL').AsString;
           Nota.Transp.Transporta.CNPJCPF := qTRANSPORTADOR.FieldbyName('FOR_CGC'         ).AsString;
           Nota.Transp.Transporta.IE      := qTRANSPORTADOR.FieldbyName('FOR_INSCRICAO'   ).AsString;
           Nota.Transp.Transporta.xEnder  := qTRANSPORTADOR.FieldbyName('FOR_ENDERECO'    ).AsString;
           Nota.Transp.Transporta.xMun    := qTRANSPORTADOR.FieldByName('FOR_MUNICIPIO'   ).AsString;
           Nota.Transp.Transporta.UF      := qTRANSPORTADOR.FieldByName('FOR_UF'          ).AsString;
        end;
    end;

    if qNaturezaOperacao.FieldByName('TPMOV_FINALIDADE').AsInteger = 1 then
    begin
        // nota fiscal complementar
        Produto := Nota.Det.Add;
        Produto.Prod.nItem := 1;
        Produto.Prod.CFOP  := qNaturezaOperacao.FieldByName('TPMOV_NATUREZAOP').AsString; // '5101';
    ...
    vOlhoNoImpostoEstadual  := 0;
             ...
             {
             if qNaturezaOperacao.FieldByname('TPMOV_SOMARICMSAOTOTALDANOTA').AsString = 'S' then
                Nota.Total.ICMSTot.vNF   := Nota.Total.ICMSTot.vNF + Nota.Total.ICMSTot.vICMS;
             if qNaturezaOperacao.FieldByname('TPMOV_SOMARIPIAOTOTALDANOTA').AsString = 'S' then
...
//##############################################################################
    ...
    //if qNaturezaOperacao.FieldByName('TPMOV_VENDA').AsString <> '9' then
    {
    case qNaturezaOperacao.FieldByName('TPMOV_FINALIDADE').AsInteger of
       0 :
       begin
          // NOTA NORMAL
...
begin
   ...
   //---------------------------------------------------------------------------
   if qNaturezaOperacao.FieldByName('TPMOV_CLIFOR').AsString = 'C' then
...
begin
   ...
   try
     ...
     qVENDA_ITEM.Sql.Add('  FROM VENDA_ITEM,                    ');
     qVENDA_ITEM.Sql.Add('       PRODUTO                        ');
     qVENDA_ITEM.Sql.Add(' WHERE CODIGO_PRODUTO = CODIGO        ');
     qVENDA_ITEM.Sql.Add('   AND CODIGO_VENDA   = :IPED_NRPEDIDO');
...
begin
   ...
   begin
     ...
     if qVENDA_ITEM.ParamByName('ICMS_CST').AsString = '' then
     begin
       ShowMessage('O produto:'+#13+#13+
                   qVENDA_ITEM.ParamByName('DESCRICAO').AsString+#13+#13+
                   'Código ['+IntToStr(qVENDA_ITEM.ParamByName('CODIGO_PRODUTO').AsInteger)+']'+#13+#13+
     ...
     begin
       ...
       try
          StrToInt(qVENDA_ITEM.ParamByName('ICMS_CST').AsString);
       except
         ShowMessage('O produto:'+#13+#13+
                     qVENDA_ITEM.ParamByName('DESCRICAO').AsString+#13+#13+
                     'Código ['+IntToStr(qVENDA_ITEM.ParamByName('CODIGO_PRODUTO').AsInteger)+']'+#13+#13+
                     'Possui um código da ST do ICMS inválido definido em seu cadastro: ['+
                     qVENDA_ITEM.ParamByName('ICMS_CST').AsString+']'#13+#13+
                     'Impossível emitir NFe. '+#13+#13+
                     'Corrija o cadastro e tente novamente.');
         result := true;
       end;
...
begin
   ...
   begin
     ...
     if qVENDA_ITEM.ParamByName('TIPO_ITEM').AsString = '' then
     begin
       ShowMessage('O produto:'+#13+#13+
                   qVENDA_ITEM.ParamByName('DESCRICAO').AsString+#13+#13+
                   'Código ['+IntToStr(qVENDA_ITEM.ParamByName('CODIGO_PRODUTO').AsInteger)+']'+#13+#13+
     ...
     begin
       ...
       if (qVENDA_ITEM.ParamByName('TIPO_ITEM').AsString <> '00') and
          (qVENDA_ITEM.ParamByName('TIPO_ITEM').AsString <> '09') then
       begin
         ShowMessage('O produto:'+#13+#13+
                     qVENDA_ITEM.ParamByName('DESCRICAO').AsString+#13+#13+
                     'Código ['+IntToStr(qVENDA_ITEM.ParamByName('CODIGO_PRODUTO').AsInteger)+']'+#13+#13+
                     'Não está configurado como Produto (00) nem Serviço (09), mas sim como: ['+
                     qVENDA_ITEM.ParamByName('TIPO_ITEM').AsString+']'#13+#13+
...
begin
   // Critica dados do Movimento
   result := false;
   if qVENDA.FieldByName('VENDA_TPMOV').AsString = '' then
   begin
     wnAlerta('Erro',
              'Tipo de Movimento não definido neste movimento.'+slinebreak + slinebreak +
...
begin
   result := false;
   if qNaturezaOperacao.FieldByName('TPMOV_NATUREZADAOPERACAO').AsString = '' then
   begin
     wnAlerta('Erro',
              'Tipo de Movimento [ ' +
              qNaturezaOperacao.FieldByName('TPMOV_CODIGO').AsString + '] '
...
begin
   ...
   //Tipo de Operação
   //0-entrada / 1-saída
   if qNaturezaOperacao.FieldByName('TPMOV_ES').AsString = 'S' then
   ...
   Tratar_Grupo_B20b_Nfe_Referenciada_ProdutorRural;
...
procedure TfrmEmissaoDeNFe.Tratar_Grupo_B20b_Nfe_Referenciada_ProdutorRural;
begin
   //---------------------------------------------------------------------------
   // LAYOUT FEDERAL
   //---------------------------------------------------------------------------
...
begin
   ...
   begin
      if qDESTINATARIO.FieldByName('CPF').AsString = '' then
         Nota.Dest.CNPJCPF := SoNumerosX(qDESTINATARIO.FieldByName('CPF').AsString);
      if qDESTINATARIO.FieldByName('CNPJ').AsString = '' then
...
//02-04-2020//vValorDoProduto := StrToFloat(MascToStr(FormatFloat('#,##0.00',(vQtde * vValorUnitario)))); // qVENDA_ITEM.FieldByName('PRECO' ).AsFloat))));//vValorUnitario * vQtde))));
             //if qNaturezaOperacao.FieldByName('TPMOV_COMOAPLICARICMS').AsString = 'N' then
...
//02-04-2020//             Produto.Prod.vDesc    := StrToFloat(MascToStr(FormatFloat('#,##0.00',(vPercentualDescontoRateadoItem / 100 * vValorDoProduto))));
             ...
             //-----------------------------------------------------------------------
             {
             if qNaturezaOperacao.FieldByName('TPMOV_COMOAPLICARICMS').AsString = 'P' then
             begin
               // ICMS VEM DO PRODUTO
               ...
               {
               if qNaturezaOperacao.FieldByName('TPMOV_ALIQ_SUBST_TRIBUTARIA').AsFloat <> 0 then
               begin
                  ...
                  begin
                     // vICMSST  := qNaturezaOperacao.FieldByName('TPMOV_ALIQ_SUBST_TRIBUTARIA').AsFloat/100 * vValorDoProduto;
                     // pRedBCST := 100 - qNaturezaOperacao.FieldByName('TPMOV_ALIQ_SUBST_TRIBUTARIA').AsFloat;
                     // AQUI AQUI AQUI
                     Produto.Imposto.ICMS.pICMS  := qVENDA_ITEM.FieldByName('PROD_PERCICMS').AsInteger; // 19;
                     vValorAuxiliar := StrToFloat(MascToStr(FormatFloat('#,##0.00',Produto.Imposto.ICMS.pICMS / 100 * vValorDoProduto)));
                     vValorDoICMS := vValorAuxiliar;
                     ...
                     //begin
                        ...
                        Produto.Imposto.ICMS.vBCST := vValorDoProduto + (vValorDoProduto * qNaturezaOperacao.FieldByName('TPMOV_ALIQ_SUBST_TRIBUTARIA').AsFloat/100);
                        vValorAuxiliar := Produto.Imposto.ICMS.vBCST;
                        vValorAuxiliar := Produto.Imposto.IPI.vIPI;
                        vValorAuxiliar := (Produto.Imposto.IPI.vIPI + vValorDoProduto) + (Produto.Imposto.IPI.vIPI + vValorDoProduto) * qNaturezaOperacao.FieldByName('TPMOV_ALIQ_SUBST_TRIBUTARIA').AsFloat/100;
                        vValorAuxiliar := 100 - qNaturezaOperacao.FieldByName('TPMOV_ALIQ_SUBST_TRIBUTARIA').AsFloat;
                        vValorAuxiliar := StrToFloat(MascToStr(FormatFloat('#,##0.00',vValorAuxiliar)));
                        Produto.Imposto.ICMS.pRedBCST := vValorAuxiliar;
                        vValorAuxiliar        := Produto.Imposto.ICMS.vBCST;
                        vValorAuxiliar        := (Produto.Imposto.ICMS.vBCST - vValorDoProduto) * (qNaturezaOperacao.FieldByName('TPMOV_ALIQ_ICMS1').AsFloat+qNaturezaOperacao.FieldByName('TPMOV_ALIQ_FOMEZERO').AsFloat)/100;
                        vValorAuxiliar        := StrToFloat(MascToStr(FormatFloat('#,##0.00',vValorAuxiliar)));
                        Produto.Imposto.ICMS.vICMSST  := vValorAuxiliar;
                        Produto.Imposto.ICMS.pMVAST   := qNaturezaOperacao.FieldByName('TPMOV_ALIQ_SUBST_TRIBUTARIA').AsFloat;
                     {end
                     else
                     begin
                        // cliente afiliado a aderj
                        vValorAuxiliar        := Produto.Imposto.IPI.vIPI;
                        vValorAuxiliar        := StrToFloat(MascToStr(FormatFloat('#,##0.00',vValorDoProduto * (qNaturezaOperacao.FieldByName('TPMOV_ALIQ_SUBST_TRIBUTARIA').AsFloat)/100)));;
                        vValorAuxiliar := (Produto.Imposto.IPI.vIPI + vValorDoProduto) + (Produto.Imposto.IPI.vIPI + vValorDoProduto) * qNaturezaOperacao.FieldByName('TPMOV_ALIQ_SUBST_TRIBUTARIA').AsFloat/100;
                        ...
                        vValorAuxiliar        := Produto.Imposto.ICMS.vBCST;
                        vValorAuxiliar        := (qNaturezaOperacao.FieldByName('TPMOV_ALIQ_ICMS2').AsFloat + qNaturezaOperacao.FieldByName('TPMOV_ALIQ_FOMEZERO').AsFloat)/100;
                        vValorAuxiliar        := (Produto.Imposto.ICMS.vBCST - vValorDoProduto) *  (qNaturezaOperacao.FieldByName('TPMOV_ALIQ_ICMS2').AsFloat + qNaturezaOperacao.FieldByName('TPMOV_ALIQ_FOMEZERO').AsFloat)/100;
             ...
             //-----------------------------------------------------------------------
             {
             if qNaturezaOperacao.FieldByName('TPMOV_COMOAPLICARICMS').AsString = 'C' then
             begin
                // ICMS VEM DO CLIENTE
                Produto.Imposto.ICMS.pICMS  := 0;
                Produto.Imposto.ICMS.vICMS  := 0;
                ...
                begin
                   ...
                   begin
                      ...
                      vValorAuxiliar := Produto.Imposto.IPI.vIPI;
                      vValorAuxiliar := StrToFloat(MascToStr(FormatFloat('#,##0.00',vValorDoProduto * (qNaturezaOperacao.FieldByName('TPMOV_ALIQ_SUBST_TRIBUTARIA').AsFloat)/100)));
                      // vValorAuxiliar := Produto.Imposto.IPI.vIPI + StrToFloat(MascToStr(FormatFloat('#,##0.00',vValorDoProduto * (qNaturezaOperacao.FieldByName('TPMOV_ALIQ_SUBST_TRIBUTARIA').AsFloat)/100)));
                      vValorAuxiliar := (Produto.Imposto.IPI.vIPI + vValorDoProduto) + (Produto.Imposto.IPI.vIPI + vValorDoProduto) * qNaturezaOperacao.FieldByName('TPMOV_ALIQ_SUBST_TRIBUTARIA').AsFloat/100;
                      Produto.Imposto.ICMS.vBCST    := vValorAuxiliar;
                      //
                      vValorAuxiliar        := 100 - qNaturezaOperacao.FieldByName('TPMOV_ALIQ_SUBST_TRIBUTARIA').AsFloat;
                      ...
                      //
                      vValorAuxiliar        := Produto.Imposto.ICMS.vBCST;
                      vValorAuxiliar        := (qNaturezaOperacao.FieldByName('TPMOV_ALIQ_ICMS1').AsFloat + qNaturezaOperacao.FieldByName('TPMOV_ALIQ_FOMEZERO').AsFloat)/100;
                      vValorAuxiliar        := (Produto.Imposto.ICMS.vBCST - vValorDoProduto) * (qNaturezaOperacao.FieldByName('TPMOV_ALIQ_ICMS1').AsFloat+qNaturezaOperacao.FieldByName('TPMOV_ALIQ_FOMEZERO').AsFloat)/100;
                      Produto.Imposto.ICMS.vICMSST  := StrToFloat(MascToStr(FormatFloat('#,##0.00',vValorAuxiliar)));
                   end
                   else
                   begin
                      ...
                      Produto.Imposto.ICMS.vBCST    := vValorAuxiliar;
                      //
                      // Produto.Imposto.ICMS.vBCST    := vBCST * qNaturezaOperacao.FieldByName('TPMOV_ALIQ_ICMS2').AsFloat / 100;
                      Produto.Imposto.ICMS.pRedBCST := StrToFloat(MascToStr(FormatFloat('#,##0.00',100 - qNaturezaOperacao.FieldByName('TPMOV_ALIQ_SUBST_TRIBUTARIA').AsFloat)));
                      Produto.Imposto.ICMS.vICMSST  := StrToFloat(MascToStr(FormatFloat('#,##0.00',(Produto.Imposto.ICMS.vBCST - vValorDoProduto) *  (qNaturezaOperacao.FieldByName('TPMOV_ALIQ_ICMS2').AsFloat+qNaturezaOperacao.FieldByName('TPMOV_ALIQ_FOMEZERO').AsFloat)/100)));
                   end;
                end;
             end; // if qNaturezaOperacao.FieldByName('TPMOV_COMOAPLICARICMS').AsString = 'C' then
             }
             Produto.Imposto.ICMS.vBC := vValorDaBaseDeCalculoDoICMS;
             // Produto.Imposto.ICMS.vICMSST :=
             //if qNaturezaOperacao.FieldByName('TPMOV_DESTACARICMS').AsInteger = 0 then
             //begin
             ...
             // 4.00--------------------------------------------------------
             //if qNaturezaOperacao.FieldByName('TPMOV_FUNDOCOMBATEPOBREZA').AsInteger = 1 then  // 4.00
             //begin
             ...
             end;  //if qNaturezaOperacao.FieldByName('TPMOV_FUNDOCOMBATEPOBREZA').AsInteger = 1 then  // 4.00
             }
             //
...
begin
   ...
   // Somente para medicamentos e matéria-prima farmacêutica
   if qVENDA_ITEM.FieldByName('NFe_Medicamento').AsInteger = 0 then
      exit;
...
begin
   ...
   // Somente para Armamentos
   if qVENDA_ITEM.FieldByName('NFe_Combustivel').AsInteger = 0 then
      exit;
       ...
       {269-Q06}
{
   ...
   begin
       ...
       begin
          ...
          if qVENDA_ITEM.FieldByName('CODIGO_REGIME_TRIBUTARIO').AsInteger = 2 then
             // Simples Nacional
             pPIS := 0.0
          else
          begin
...
begin
   ...
   // parei aqui 22/04/20 **********
   // vou tratar grupo Y – Dados da Cobrança
   // Página 153 de 232
...
begin
   ...
   begin
     ...
     UFST := qDESTINATARIO.FieldByName('ESTADO').AsString;
...
begin
     ...
     if (qVENDA_ITEM.FieldByName('ICMS_CST').AsString <> '10') and
        ...
        (qVENDA_ITEM.FieldByName('ICMS_CST').AsString <> '70') then
        exit;
...
begin
   if qDESTINATARIO.FieldByName('CODIGO_REGIME_TRIBUTARIO').AsString <> '1' then
      exit;

   if qDESTINATARIO.FieldByName('NFe_CSOSN').AsString = '' then
      vNFe_CSOSN := 000
   else
   begin
       try
         vNFe_CSOSN := StrToInt(qDESTINATARIO.FieldByName('NFe_CSOSN').AsString);
       except
       end;
   end;
