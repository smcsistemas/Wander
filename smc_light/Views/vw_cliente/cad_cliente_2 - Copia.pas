...
|-----|--------|--------------------|------------------------------------------|
|  121|05/05/20|Wander              |Inicio da Implementação do código de cad  |                                          |
|     |   18:28|                    |cliente no novo layout                    |
|-----|--------|--------------------|------------------------------------------|
|  122|05/05/20|Wander              |Os dois botões (Pessoa Física) e (Pessoa  |
|     |   19:22|                    |Jurídica) foram substituídos por um TRadio|
|     |        |                    |group com as opções Física e Jurídica     |
|-----|--------|--------------------|------------------------------------------|
|  123|05/05/20|Wander              |Eliminadas as sobreposições dos grupos    |
...
<<<<<<< HEAD
    ...
    tFiscal: TcxTabSheet;
    rgCRT_CODIGO_REGIME_TRIBUTARIO: TRadioGroup;
  ...
  private
    ...
    procedure So_Pode_Incluir_e_Alterar;
    procedure Pode_Alterar;
    procedure So_Pode_Cancelar_ou_Gravar;
    ...
    procedure Mostrar_Contatos_Proprietário;
...
var NaoPesquisar: Boolean; // Impede que haja pesquisas enquanto a tela está
...
begin
 ...
 vPesquisaExterna := True; // Informa a frm_autitoria que ela não está sendo
...
begin
    ...
    ShowMessage('Cliente bloqueado em '
                + qConsultaDTBLOQUEIO.AsString + ' - '
                + qConsultaHRBLOQUEIO.Value + #13 + 'pelo usuário [ '
                + qConsultaUSUBLOQUEIO.Value + ' ] ' + #13 + 'na estação [ '
...
procedure Tfrm_cadastro_cliente.bt_Cliente_AlterarClick(Sender: TObject);
begin
   if not TemAcesso(Global_Usuario_Logado, 'ALTCLI') then
     exit;

   //Permitir edição dos campos da tela
   HabilitarCampos(True);

   So_Pode_Cancelar_ou_Gravar;

   //Posiciona no 1o campo da tela
   rgPessoa.SetFocus;
end;

procedure Tfrm_cadastro_cliente.bt_Cliente_CancelarClick(Sender: TObject);
begin
   //limpar todos os campos da tela
   Limpa_Tela;

   //Impedir edição dos campos da tela
...
procedure Tfrm_cadastro_cliente.bt_Cliente_GravarClick(Sender: TObject);
begin
...
procedure Tfrm_cadastro_cliente.bt_Cliente_IncluirClick(Sender: TObject);
begin
...
begin
    if bt_Cliente_Cancelar.focused then exit;
...
begin
   ...
   begin
     ShowMessage('Ramo de Atividade não encontrado');
...
begin
   ...
   // Destacar com cor cinza claro campos obrigatórios
   Destacar_Campos_Obrigatorios(_Cor_CampoObrigatorio);

   // Pessoa Física
   rgPessoa.ItemIndex := 0;

   //Preencher opções de ordenamento dos registros da consulta
   ...
   begin
      ...
      Add('Cidade');
      Add('UF');
      // padrão ordenar por nome fantasia
      cbOrder.ItemIndex := 2;
   end;
   ...
   // Padrão filtrar apenas os clientes ativos
   rgSTATUS_CADASTRAL.ItemIndex := 0;

   // Padrão filtrar Ativos
   rgSTATUS_CADASTRAL.ItemIndex := 0;

   // Padrão filtrar todos
...
begin
   ...
   begin
      ShowMessage('Este CPF pertence ao cliente '+#13+#13+
...
begin
   ...
   if not qAUX.Eof then
   begin
      ShowMessage('Este CNPJ pertence ao cliente '+#13+#13+
                  qAUX.FieldByName('RAZAO_SOCIAL').AsString);
      result := True;
   end;
   qAUX.Free;
...
edCODIGO.Text := qConsulta.FieldByName('CODIGO').AsString;
   ...
   begin
     //Cliente ativo: Não mostrar quando foi inativado
...
procedure Tfrm_cadastro_cliente.Mostrar_Contatos_Proprietário;
begin
...
begin
   // Carregar Campos dos endredeços do cliente
   //---------------------------------------------------------------------------
   pcEnderecos.ActivePageIndex:=0;
...
procedure Tfrm_cadastro_cliente.So_Pode_Cancelar_ou_Gravar;
begin
   //Desabilita [Incluir] / [Alterar]
   bt_Cliente_Incluir.Enabled := false;
   bt_Cliente_Alterar.Enabled := false;

   //Habilita [Cancelar] / [Gravar]
...
begin
   // Controle de botões que tratam o cadastro do cliente
   //---------------------------------------------------------------------------
...
procedure Tfrm_cadastro_cliente.So_Pode_Incluir_e_Alterar;
begin
   //Desabilita [Incluir] / [Alterar]
   bt_Cliente_Incluir.Enabled := True;
   bt_Cliente_Alterar.Enabled := True;
...
begin
   ...
   //---------------------------------------------------------------------------
   // Clientes ativos aparecerão em verde
   // Clientes inativos aparecerão em vermelho
   //---------------------------------------------------------------------------
...
begin
   ...
   if SL.Count > 0 then
      ShowMessage('Informe o campos obritatórios: '+#13+#13
                  + SL.Text)
   else
      result := true;
   SL.Free;
