...
type
  Tfrm_cliente = class(TForm)
    ...
    Label18: TLabel;
    edID_CAD_RAMO_ATIVIDADE: TDBEdit;
    ...
    Procedure SQL_C_ClientesAfterScroll(DataSet: TDataSet);
    ...
    procedure edID_CAD_RAMO_ATIVIDADEChange(Sender: TObject);
...
Procedure Tfrm_cliente.bt_Cliente_GravarClick(Sender: TObject);
  var
    x: Integer;
  begin
    if Bt_Cliente_Incluir.Visible = false then
    begin
      ...
      TRY { Inicia cadastro }
        ...
        begin
          ...
          begin
            if simplequery('SELECT CPF_CAD_CLIENTE FROM PARAMETROS_SISTEMA')
              .Fields[0].AsString = m_true then
            begin
              ...
              ('SELECT CPF_CAD_CLIENTE FROM PARAMETROS_SISTEMA')
              .Fields[0].AsString = m_false then
            begin
              if CamposObrigatorios([dbedtNome, edt_ie],
                ['Nome', 'IE * ISENTO - Caso não possua'], [], [], [], [],
                [], []) then
          ...
          begin
            if simplequery('SELECT CPF_CAD_CLIENTE FROM PARAMETROS_SISTEMA')
              .Fields[0].AsString = m_true then
            begin
              if u_funcoes.CamposObrigatorios([dbedtNome], ['Nome'], [], [], [],
                [], [edCPF], ['CPF']) then
              begin
      ...
      FINALLY
        ...
        if Cadastrando_ou_Alterando = 'Cadastrando' Then
        begin
...
Procedure Tfrm_cliente.bt_Cliente_IncluirClick(Sender: TObject);
  begin
    ...
    if not Bt_Cliente_Incluir.Visible then
      exit;
...
Procedure Tfrm_cliente.ConsultaParaCadastro;
  begin
    SQL_Cliente.Close;
    SQL_Cliente.ParamByName('pcodigo').Value :=
      SQL_C_Clientes.fieldbyname('CODIGO').Value;
    SQL_Cliente.Open;
    CarregarDados;
    tab_cadastrar.Show;
  end;
...
Procedure Tfrm_cliente.btnMunicipClick(Sender: TObject);
  ...
  begin
    ...
    qEndereco_Municipio_Estado.ParamByName('CODIGO').AsInteger :=
      Codigo_Municipio;
    ...
    sql_LISTA_ENDERECOUF.AsString := qEndereco_Municipio_Estado.FieldByName
      ('UF').AsString;
    sql_LISTA_ENDERECOCODIGO_UF.AsString :=
      IntToStr(qEndereco_Municipio_Estado.FieldByName('IBGE').AsInteger);
...
Procedure Tfrm_cliente.bt_Endereco_IncluirClick(Sender: TObject);
  begin
{
    AlterarEnabled([gpEndereco, btn_cep, btnMunicip, btn_zona, DBComboBox1,
      DBComboBox5, bt_Endereco_Alterar, bt_Endereco_Cancelar,
      btn_bairro], true);
}
...
Procedure Tfrm_cliente.Abrir_tabelas_relacionadas;
  begin
    ...
    sql_LISTA_ENDERECO.ParamByName('CODIGO_CLIENTE').AsInteger :=
      SQL_ClienteCODIGO.Value;
    ...
    if sql_LISTA_ENDERECO.FieldByName('STATUS_CADASTRAL').AsString = 'ATIVO'
    then
...
Procedure Tfrm_cliente.SQL_C_ClientesAfterScroll(DataSet: TDataSet);
begin
   Carrega_Enderecos;
end;
...
Procedure Tfrm_cliente.Preencher_Tela_Com_Dados_da_Consulta_CNPJ;
  begin
    ...
    SQL_ClienteRAZAO_SOCIAL.Value :=
      SemAsteriscos(frmConsultaCNPJ.EditRazaoSocial.Text);
    ...
    begin
      ...
      sql_CLIENTE_ENDERECORUA.Value :=
        SemAsteriscos(frmConsultaCNPJ.EditEndereco.Text);
      sql_CLIENTE_ENDERECONUMERO.Value :=
        SemAsteriscos(frmConsultaCNPJ.EditNumero.Text);
      sql_CLIENTE_ENDERECOBAIRRO.Value :=
        SemAsteriscos(frmConsultaCNPJ.EditBairro.Text);
      sql_CLIENTE_ENDERECOCOMPLEMENTO.Value :=
        SemAsteriscos(frmConsultaCNPJ.EditComplemento.Text);
      sql_CLIENTE_ENDERECOCEP.Value :=
        SemAsteriscos(frmConsultaCNPJ.EditCEP.Text);
      sql_CLIENTE_ENDERECOMUNICIPIO.Value :=
        SemAsteriscos(frmConsultaCNPJ.EditCidade.Text);
      sql_CLIENTE_ENDERECOCODIGO_MUNICIPIO.Value :=
        CodigoMunicipio(frmConsultaCNPJ.EditCidade.Text,
        frmConsultaCNPJ.EditUF.Text);
      sql_CLIENTE_ENDERECOUF.Value :=
        SemAsteriscos(frmConsultaCNPJ.EditUF.Text);
      sql_CLIENTE_ENDERECOCODIGO_UF.Value :=
        CodigoUF(frmConsultaCNPJ.EditUF.Text);
...
Procedure Tfrm_cliente.cbATIVIDADEClick(Sender: TObject);
  begin
    ...
    SQL_ClienteID_CAD_RAMO_ATIVIDADE.Value :=
      FRM_cad_atividade.SQL_LISTACODIGO.Value;
    FRM_cad_atividade.Free;
  end;
...
Procedure Tfrm_cliente.btn_bairroClick(Sender: TObject);
  begin
    ...
    DB_BAIRRO.Text := frm_cad_bairro.SQL_LISTADESCRICAO.value;
...
begin
   ...
   edLimiteCredito.text := Float_to_String(vValor);
end;
...
begin
  ...
  DB_ID_REGIAO.Text := Frm_regiao.SQL_REGIOES.FieldByName('REG_CODIGO').AsString;
  Frm_regiao.Free;
end;
...
procedure Tfrm_cliente.edID_CAD_RAMO_ATIVIDADEChange(Sender: TObject);
  begin
    dbatividade.Text := DescricaoCAD_RAMO_ATIVIDADE
      (SQL_ClienteID_CAD_RAMO_ATIVIDADE.Value);
  end;

procedure Tfrm_cliente.edID_CAD_RAMO_ATIVIDADEExit(Sender: TObject);
begin
...
begin
   ...
   if UF     <> '' then db_UF.Text        := UF;
   if CIDADE <> '' then db_MUNICIPIO.Text := CIDADE;
   if RUA    <> '' then db_RUA.Text       := RUA;
   if BAIRRO <> '' then db_BAIRRO.Text    := BAIRRO;
end;
