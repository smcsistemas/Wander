...
type
  ...
  const
    ...
    PRODUTO_QUERY_BASE =
      'SELECT PROD_CODIGO, PROD_EAN, PROD_DESCRICAO, PROD_CDUNIDADE, PROD_SALDO, PROD_REFERENCIASFABRICA, MARCA, PROD_PRECO_VAR FROM PRODUTO_PROD WHERE STATUS_CADASTRAL <> "INATIVO"';
...
begin
  ...
  TCurrencyField(SQL_DATA.FieldByName('PROD_PRECO_VAR')).currency := true;
end;
...
begin
  case cgTable of
    cgProduto:
      result := ' ORDER BY ' + f(['CODIGO', 'PROD_EAN', 'PROD_DESCRICAO', 'PROD_CDUNIDADE', 'PROD_SALDO', 'PROD_REFERENCIASFABRICA',
        'MARCA', 'PROD_PRECO_VAR']);
    cgFornecedor:
