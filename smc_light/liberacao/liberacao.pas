...
|----------|-------------|-----------------------------------------------------|
|21/02/2020|WANDER       |Passou a tratar dados criptografados do usuário.     |
|        15|             |                                                     |
================================================================================
}
...
begin
  ...
  begin
    wnAlerta('Erro no Login', slinebreak + slinebreak + 'Campo de Usuário deve ser preenchido!');
    edt_usuario.SetFocus;
    exit;
  end;
  // Wander
  // Criação do usuario MASTER com senha PIPOCA
  //----------------------------------------------------------------------------
  if UsuarioMASTER_SenhaCorreta(edt_usuario.text,edt_Senha.Text) then
  begin
     RegistraLog('Acessou Configurações Administrativas');
     ModalResult := mrOk;
     exit;
  end;
  if edt_usuario.text = 'MASTER' then
  begin
    wnAlerta('Permissões', slinebreak + slinebreak + 'Senha incorreta! Tente novamente.');
    edt_usuario.SetFocus;
    edt_usuario.SelectAll;
    exit;
  end;

  // Fim Criação do usuario MASTER com senha PIPOCA
  //----------------------------------------------------------------------------
  ...
  try
      ...
      begin
        wnAlerta('Permissões', slinebreak + slinebreak + 'Usuário não cadastrado :/');
        edt_usuario.SetFocus;
        edt_usuario.SelectAll;
        exit;
      end;
      ...
      begin
        wnErro('Permissões', slinebreak + slinebreak + 'O usuário informado não tem permissões suficientes para executar essa ação!');
        edt_usuario.SetFocus;
        exit;
      end;
      ...
      On E: Exception do
      begin
        wnErro('Erro de Conexão.', E.Message + System.slinebreak + 'Erro de Conexão com o Banco de Dados.');
        ModalResult := mrAbort;
      end;
    end;
end;
