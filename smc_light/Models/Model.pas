...
uses FireDAC.Comp.client, h_db, System.Generics.Collections, System.Rtti, SysUtils, h_Functions, data.db, System.TypInfo, System.DateUtils,
  strutils;
...
begin
  ...
  if pk <> 0 then
  begin
    qry := TDB.SimpleQuery('select * from ' + TModel.getTableName(Self.className) + ' where id = ? ', [pk]);
    if qry <> nil then
    begin
...
begin
  Self.aClassTypeInfo := TModel.getContext.GetType(Self.ClassType);
  TDB.ExecQuery('delete from ' + TModel.getTableName(Self.className) + ' where id = ? ', [Self.aClassTypeInfo.GetField(ID_FIELD).GetValue(Self).asInteger]);
end;
...
begin
  result := false;
  qry := TDB.SimpleQuery('select * from ' + TModel.getTableName(Self.className) + ' ' + TDB.GenWhere(fields) + ' order by id limit 1', values);
  if qry <> nil then
...
begin
  result := nil;
  qry := TDB.SimpleQuery('select * from ' + TModel.getTableName(Self.className) + ' ' + TDB.GenWhere(fields), values);
  if qry <> nil then
  begin
    qry.first;
...
begin
  ...
  begin
    ...
    sql := 'insert into ' + TModel.getTableName(Self.className) + '(id,' + Tfunctions.arrToStr(fieldNames.ToArray) + ') values(default ' +
      Tfunctions.genData(',?', length(fieldValues.ToArray)) + ')';

    TDB.ExecQuery(sql, fieldValues.ToArray);

    Self.aClassTypeInfo.GetField(ID_FIELD).SetValue(Self, TDB.SimpleQuery('select id from ' + TModel.getTableName(Self.className) + ' order by id desc limit 1')
      .fields[0].asInteger);
