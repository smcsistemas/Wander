...
var
  Frm_Conf_Nfceok: TFrm_Conf_Nfceok;
...
procedure TFrm_Conf_Nfceok.PadronizarLayout;
var
  qry_tmp: TFDQuery;
begin
...
procedure TFrm_Conf_Nfceok.btnGravarClick(Sender: TObject);
begin
  try
...
procedure TFrm_Conf_Nfceok.btnSairClick(Sender: TObject);
begin
...
procedure TFrm_Conf_Nfceok.btn_AlterarClick(Sender: TObject);
begin
...
procedure TFrm_Conf_Nfceok.btn_buscar_xmlClick(Sender: TObject);
begin
  edtPastaNotas.Text := tFunctions.SelectFolder;
end;

procedure TFrm_Conf_Nfceok.cb_certificadoEnter(Sender: TObject);
begin
  if ExisteCert then
    Frm_ConfNFCE.spdNFCe.ListarCertificados(cb_certificados.Properties.Items);
end;

procedure TFrm_Conf_Nfceok.cb_certificadosExit(Sender: TObject);
begin
  DataCertificado;
end;

procedure TFrm_Conf_Nfceok.chk_contingenciaPropertiesChange(Sender: TObject);
begin
  u_funcoes.AlteraChecks(chk_contingencia, [chk_normal]);
end;

procedure TFrm_Conf_Nfceok.chk_danfe_diretoPropertiesChange(Sender: TObject);
begin
...
procedure TFrm_Conf_Nfceok.chk_homologacaoPropertiesChange(Sender: TObject);
begin
  u_funcoes.AlteraChecks(chk_homologacao, [chk_producao]);
end;

procedure TFrm_Conf_Nfceok.chk_normalPropertiesChange(Sender: TObject);
begin
  u_funcoes.AlteraChecks(chk_normal, [chk_contingencia]);
end;

procedure TFrm_Conf_Nfceok.chk_producaoPropertiesChange(Sender: TObject);
begin
  u_funcoes.AlteraChecks(chk_producao, [chk_homologacao]);
end;

procedure TFrm_Conf_Nfceok.DataCertificado;
begin
  if ExisteCert then
...
function TFrm_Conf_Nfceok.ExisteCert: boolean;
var
  certs: tstringlist;
begin
...
procedure TFrm_Conf_Nfceok.btn_certificado_serieClick(Sender: TObject);
begin
  Frm_ConfNFCE.spdNFCe.ListarCertificados(cb_certificados.Properties.Items);
end;

procedure TFrm_Conf_Nfceok.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := cafree;
  self := nil;
end;

procedure TFrm_Conf_Nfceok.FormShow(Sender: TObject);
begin
  PadronizarLayout;
end;

procedure TFrm_Conf_Nfceok.id_tokenExit(Sender: TObject);
begin
  id_token.Text := FormatFloat('000000', strtofloat(id_token.Text));
end;
