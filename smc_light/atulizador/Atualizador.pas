...
{       Module.Query.Close;
   ...
   if fNaoAtualizado('Produto: Indicador de Medicamento...') then
    ...
    if fNaoAtualizado('PROD_....') Then
    begin
       // O custo pode ser:
       //                  médio
       //                  último ou
       //                  informado
       //E ainda pode ser Símples (Nominal) ou Composto (Real)
       //E deve ser fornecido por uma função que trate movimentos
       //Portanto foi eliminado do cadastro do produto
       Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN PRECO_CUSTO');
       Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN CUSTO_MEDIO');

       //o Frete varia com o movimento (distancia)
       //Não faz sentido estar no cad do prod
       //Eliminado
       Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN FRETE');

       //Dinamico... Será a margem desejada? Pouco produto funciona desta forma
       //Normalmente é o mercado que dita o preço (lei da oferta e procura)
       //Será para estimar o preço de venda?
       Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN MARGEM_LUCRO');
       Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN MARGEM_L_VAREJO');
       Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN MARGEM_L_ATACADO');
       Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN MARGEM_L_DISTRIBUIDOR');

       //O Imposto é variável. Seria o total de impostos que incidem sobre o
       //produto? mesmo assim, também varia. Depende de quem se compra, por
       //exemplo....
       Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN IMPOSTO');
       //
       //O mesmo
       Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN DESP_OPERACIONAIS');

    end;
    if fNaoAtualizado('decimais......') Then
    begin
       Executar('ALTER TABLE PRODUTO_PROD ADD PROD_ESTOQMIN          DECIMAL(15,4) NULL DEFAULT 0.0  COMMENT "Quantidade de Estoque Mínima"        ');
       Executar('ALTER TABLE PRODUTO_PROD ADD PROD_MAXDESC           DECIMAL(10,4) NULL DEFAULT 0.0  COMMENT "Perc % max de desconto"              ');
       Executar('ALTER TABLE PRODUTO_PROD ADD PROD_MAXDESC_VAR       DECIMAL(10,4) NULL DEFAULT 0.0  COMMENT "Perc % max desconto varejo"          ');
       Executar('ALTER TABLE PRODUTO_PROD ADD PROD_MAXDESC_ATAC      DECIMAL(10,4) NULL DEFAULT 0.0  COMMENT "Perc % max desconto atacado"         ');
       Executar('ALTER TABLE PRODUTO_PROD ADD PROD_MAXDESC_DIST      DECIMAL(10,4) NULL DEFAULT 0.0  COMMENT "Perc % max desconto Distribuição"    ');
       Executar('ALTER TABLE PRODUTO_PROD ADD PROD_COMISSAO          DECIMAL(10,4) NULL DEFAULT 0.0  COMMENT "Perc % Comissão"                     ');
       Executar('ALTER TABLE PRODUTO_PROD ADD PROD_COMISSAO_LOJA     DECIMAL(10,4) NULL DEFAULT 0.0  COMMENT "Perc % Comissão Loja"                ');
       Executar('ALTER TABLE PRODUTO_PROD ADD PROD_COMISSAO_VAR      DECIMAL(10,4) NULL DEFAULT 0.0  COMMENT "Perc % Comissão Varejo"              ');
       Executar('ALTER TABLE PRODUTO_PROD ADD PROD_COMISSAO_ATAC     DECIMAL(10,4) NULL DEFAULT 0.0  COMMENT "Perc % Comissão Atacado"             ');
       Executar('ALTER TABLE PRODUTO_PROD ADD PROD_COMISSAO_DIST     DECIMAL(10,4) NULL DEFAULT 0.0  COMMENT "Perc % Comissão Distribuição"        ');
       Executar('ALTER TABLE PRODUTO_PROD ADD PROD_COMISSAO_EXT      DECIMAL(10,4) NULL DEFAULT 0.0  COMMENT "Perc % Comissão Externa"             ');
       Executar('ALTER TABLE PRODUTO_PROD ADD PROD_COMISSAO_EXT_VAR  DECIMAL(10,4) NULL DEFAULT 0.0  COMMENT "Perc % Comissão Externa Varejo"      ');
       Executar('ALTER TABLE PRODUTO_PROD ADD PROD_COMISSAO_EXT_ATAC DECIMAL(10,4) NULL DEFAULT 0.0  COMMENT "Perc % Comissão Externa Atacado"     ');
       Executar('ALTER TABLE PRODUTO_PROD ADD PROD_COMISSAO_EXT_DIST DECIMAL(10,4) NULL DEFAULT 0.0  COMMENT "Perc % Comissão Externa Distribuidor"');
       Executar('ALTER TABLE PRODUTO_PROD ADD PROD_PRECO_VAR         DECIMAL(10,4) NULL DEFAULT 0.0  COMMENT "Preço de Varejo"                     ');
       Executar('ALTER TABLE PRODUTO_PROD ADD PROD_PRECO_ATAC        DECIMAL(10,4) NULL DEFAULT 0.0  COMMENT "Preço de Atacado"                    ');
       Executar('ALTER TABLE PRODUTO_PROD ADD PROD_PRECO_DIST        DECIMAL(10,4) NULL DEFAULT 0.0  COMMENT "Preço de Distribuidor"               ');

       Executar('UPDATE PRODUTO_PROD SET PROD_ESTOQMIN          = ESTOQUE_MINIMO               ');
       Executar('UPDATE PRODUTO_PROD SET PROD_MAXDESC           = DESCONTO_MAXIMO              ');
       Executar('UPDATE PRODUTO_PROD SET PROD_MAXDESC_VAR       = DESCONTO_M_VAREJO            ');
       Executar('UPDATE PRODUTO_PROD SET PROD_MAXDESC_ATAC      = DESCONTO_M_ATACADO           ');
       Executar('UPDATE PRODUTO_PROD SET PROD_MAXDESC_DIST      = DESCONTO_M_DISTRIBUIDOR      ');
       Executar('UPDATE PRODUTO_PROD SET PROD_COMISSAO          = PAGAR_COMISSAO               ');
       Executar('UPDATE PRODUTO_PROD SET PROD_COMISSAO_LOJA     = COMISSAO_BALCAO              ');
       Executar('UPDATE PRODUTO_PROD SET PROD_COMISSAO_VAR      = BALCAO_COMISSAO_VAREJO       ');
       Executar('UPDATE PRODUTO_PROD SET PROD_COMISSAO_ATAC     = BALCAO_COMISSAO_ATACADO      ');
       Executar('UPDATE PRODUTO_PROD SET PROD_COMISSAO_DIST     = BALCAO_COMISSAO_DISTRIBUIDOR ');
       Executar('UPDATE PRODUTO_PROD SET PROD_COMISSAO_EXT      = COMISSAO_EXTERNA             ');
       Executar('UPDATE PRODUTO_PROD SET PROD_COMISSAO_EXT_VAR  = EXTERNA_COMISSAO_VAREJO      ');
       Executar('UPDATE PRODUTO_PROD SET PROD_COMISSAO_EXT_ATAC = EXTERNA_COMISSAO_ATACADO     ');
       Executar('UPDATE PRODUTO_PROD SET PROD_COMISSAO_EXT_DIST = EXTERNA_COMISSAO_DISTRIBUIDOR');
       Executar('UPDATE PRODUTO_PROD SET PROD_PRECO_VAR         = PRECO_FINAL_VAREJO           ');
       Executar('UPDATE PRODUTO_PROD SET PROD_PRECO_ATAC        = PRECO_FINAL_ATACADO          ');
       Executar('UPDATE PRODUTO_PROD SET PROD_PRECO_DIST        = PRECO_FINAL_DISTRIBUIDOR     ');
    end;

    if fNaoAtualizado('Decimais 2') Then
    begin
       Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN ESTOQUE_MINIMO               ');
       Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN DESCONTO_MAXIMO              ');
       Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN DESCONTO_M_VAREJO            ');
       Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN DESCONTO_M_ATACADO           ');
       Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN DESCONTO_M_DISTRIBUIDOR      ');
       Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN PAGAR_COMISSAO               ');
       Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN COMISSAO_BALCAO              ');
       Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN BALCAO_COMISSAO_VAREJO       ');
       Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN BALCAO_COMISSAO_ATACADO      ');
       Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN BALCAO_COMISSAO_DISTRIBUIDOR ');
       Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN COMISSAO_EXTERNA             ');
       Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN EXTERNA_COMISSAO_VAREJO      ');
       Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN EXTERNA_COMISSAO_ATACADO     ');
       Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN EXTERNA_COMISSAO_DISTRIBUIDOR');
       Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN PRECO_FINAL_VAREJO           ');
       Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN PRECO_FINAL_ATACADO          ');
       Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN PRECO_FINAL_DISTRIBUIDOR     ');
    end;
    if fNaoAtualizado('Preço final_') Then
    begin
       Executar('ALTER TABLE PRODUTO_PROD ADD PROD_PROMOCAO_VAR  DECIMAL(10,4) NULL DEFAULT 0.0  COMMENT "Preço Promocional Varejo"');
       Executar('ALTER TABLE PRODUTO_PROD ADD PROD_PROMOCAO_ATAC DECIMAL(10,4) NULL DEFAULT 0.0  COMMENT "Preço Promocional Atacado"');
       Executar('ALTER TABLE PRODUTO_PROD ADD PROD_PROMOCAO_DIST DECIMAL(10,4) NULL DEFAULT 0.0  COMMENT "Preço Promocional Atacado"');

       Executar('UPDATE PRODUTO_PROD SET PROD_PROMOCAO_VAR  = PROMO_VAREJO');
       Executar('UPDATE PRODUTO_PROD SET PROD_PROMOCAO_ATAC = PROMO_ATACADO');
       Executar('UPDATE PRODUTO_PROD SET PROD_PROMOCAO_DIST = PROMO_DISTRIBUIDOR');
...
//REDUCAO_ICMS_ST PROD_NFe_N20_pRedBCST
   ...
   if fNaoAtualizado('Eliminando Código Alfanumérico Alternativo.') Then
      Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN CODIGO_ALFANUMERICO');

   if fNaoAtualizado('Eliminando Código de Localização.') Then
      Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN CODIGO_LOCALIZACAO');

   //Tributação do ICMS
   //---------------------------------------------------------------------------
   if fNaoAtualizado('Tributação do ICMS...') Then
   begin
      Executar('ALTER TABLE PRODUTO_PROD ADD PROD_NFe_N12_CST_ICMS VARCHAR(02) NULL COMMENT "Código da Situação Tributária do ICMS"');
      Executar('UPDATE PRODUTO_PROD SET PROD_NFe_N12_CST_ICMS = ICMS_CST');
      Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN ICMS_CST');
   end;

   //Tributação do IPI
   //---------------------------------------------------------------------------
   if fNaoAtualizado('Tributação do IPI....') Then
   begin
      Executar('ALTER TABLE PRODUTO_PROD ADD PROD_NFe_O09_CST_IPI VARCHAR(02) NULL COMMENT "Código da Situação Tributária do IPI"');
      Executar('UPDATE PRODUTO_PROD SET PROD_NFe_O09_CST_IPI = CST_IPI');
      Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN CST_IPI');
   end;
   ...
   // Depende da operação,
   // portanto virá da tabela: RELACAO_CFOP_x_PRODUTO_xCST_PISCOFINS_RPC
   //                  coluna: RPC_PIS
   // e não do cadastro do prodo
   if fNaoAtualizado('Eliminando PIS_CST....') Then
      Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN PIS_CST');

   //CST do COFINS : idem ao anterior
   if fNaoAtualizado('Eliminando COFINS_CST....') Then
      Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN COFINS_CST');

    {N11}
    //orig
    //Origem da mercadoria:
    //---------------------------------------------------
   if fNaoAtualizado('Origem da mercadoria...') Then
   begin
      Executar('ALTER TABLE PRODUTO_PROD ADD PROD_NFe_N11_orig integer NULL COMMENT "Código da Origem da mercadoria"');
      Executar('UPDATE PRODUTO_PROD SET PROD_NFe_N11_orig = CODIGO_ORIGEM_MERCADORIA');
      Executar('ALTER TABLE PRODUTO_PROD DROP COLUMN CODIGO_ORIGEM_MERCADORIA');
   end;
