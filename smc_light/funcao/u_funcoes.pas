...
|----------|-------------|-----------------------------------------------------|
|27/02/2020|WANDER       |Criada a função fnatureza_operacaoDESCRICAO          |
|        44|             |que recebe o ID de uma Natureza de Operação e retorna|
|          |             |sua descrição, se houver, e vazio se não houver.     |
|----------|-------------|-----------------------------------------------------|
|27/02/2020|WANDER       |Criada a função fColaboradorNOME que recebe o codigo |
|        47|             |de um Colaborador e retorna seu nome, se houver, e   |
|          |             |vazio se não houver.                                 |
================================================================================
| ITEM|DATA  HR|DESENVOLVEDOR/UNIT  |HISTORICO                                 |
|-----|--------|--------------------|------------------------------------------|
|  127|11/05/20|wander              |Criada variável Global_Filial_Em_Uso      |
...
var
  ...
  Global_Filial_Em_Uso :String; // Wander : Código (ID) da Filial em uso (ativa)
...
var Q : tFDQuery;
begin
    // Recebe o ID de uma natureza de operação e
    // retorna sua descrição, se existir ou "" se não existir
    //--------------------------------------------------------------------------
    result := '';
    q := TFDQuery.Create(nil);
    q.Connection     := Module.connection;
...
begin
    ...
    // retorna seu nome, se existir ou "" se não existir
    //--------------------------------------------------------------------------
    result := '';
    q := TFDQuery.Create(nil);
    q.Connection     := Module.connection;
...
begin
   ...
   try
     ...
     if Q.FieldByName('QTDE').AsInteger > 1 then
        ...
        result := False; // Não possui Filiais
   except
   end;
   Q.Free;
end;
